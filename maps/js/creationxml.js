...
// change your settings here : http://www.macromedia.com/support/documentation/en/flashplayer/help/settings_manager04.html
					...
					filename: function(){
						return document.getElementById("xmlfilename").value+".xml";
					},
					data: function(){ 
						return jsoncreate(); 
...
var fileInput = document.querySelector('#name_file');
 var reader = new FileReader();
if(document.getElementById("site").value=='Selection' || fileInput.files[0]==undefined || instrum=="Selection" || document.getElementById("xmlfilename").value==''){ //verify every field is filled
...
var g=k+1;
			xml +=" 	<jobTemplate id='tool0"+g+"'> \n \
		<streamingExecutable>/application/tool0"+g+"/run</streamingExecutable> \n\
		<defaultParameters> \n\
			<parameter id='filename'>"+fileInput.files[k].name+"</parameter> \n\
			<parameter id='instrument'>"+instrum+"</parameter> \n\
			<parameter id='region_name'>"+document.getElementById("site").value+"</parameter> \n\
			<parameter id='region_coords'>"+coordscalcul(document.getElementById("site").value)+"</parameter> \n\
		</defaultParameters> \n\
	</jobTemplate>\n";
...
function coordscalcul (value){ //create the part of the xml which concerns the coords
var result='[';
switch(value){
case 'Uyuni':
...
if ( fileInput.files[0]== undefined){
		document.getElementById('message').innerHTML="Error: no xml file found"; // if you click on the button while nothing is inside the file object
		document.getElementById('message').style.color="red";
		document.getElementById('message').style.visibility="visible";
}else {var jsonname= fileInput.files[0].name;
	var jsontest=jsonname.split('.');
		if (jsontest[(jsontest.length)-1]!='xml' ){ 
		document.getElementById('message').innerHTML="Error: It's not a xml file"; //check if the uploaded file is a json
		document.getElementById('message').style.color="red";
		document.getElementById('message').style.visibility="visible";
		}
		else{
